...
/**
 * AdvancedImageManipulationModel is a class that extends ImageManipulationModel and implements
 * AdvancedImageManipulationInterface. It provides advanced image manipulation features such as
 * blur, sharpen, greyscale, sepia tone, and dithering.
 */
...
public class AdvancedImageManipulationModel extends ImageManipulationModel
  ...
  public ImageInterface sharpen(ImageInterface image) {
    ImageInterface sharpenImage = new Image(image.getHeight(), image.getWidth(),
            image.getMaxValue());
    ...
    int row = image.getHeight();
    int col = image.getWidth();
    PixelInterface[][] sharpenPixel = new Pixel[row][col];
    for (int i = 0; i < row; i++) {
      for (int j = 0; j < col; j++) {
  ...
  private int multiplyMatrix(PixelInterface pixel, double[] doubles) {
    return (int) (pixel.getRed() * doubles[0] + pixel.getGreen() * doubles[1]
            + pixel.getBlue() * doubles[2]);
  }
  ...
  public ImageInterface dither(ImageInterface image) {
    ...
    ImageInterface ditheredImage = new Image(greyImage.getHeight(), greyImage.getWidth(),
            greyImage.getMaxValue());
    ...
    for (int r = 0; r < greyImage.getHeight(); r++) {
      for (int c = 0; c < greyImage.getWidth(); c++) {
        int oldColor = greyPixels[r][c].getRed(); // red, green, and blue are the same since
        // it's greyscale
  ...
  private double applyBlurOnBlueChannel(PixelInterface[][] originalPixel, int i, int j, int rowEnd,
                                        int colEnd) {
    Set<String> set = new HashSet<>();
    return computeValue(originalPixel, 1,
            1, i, j, rowEnd, colEnd, blur_filter.length, blur_filter[0].length, set, 2,
            blur_filter);

  }

  private double computeValue(PixelInterface[][] originalPixel, int filterRow, int filterCol,
                              int pixelRow,
                              int pixelCol,
    ...
    return filter[filterRow][filterCol] * channel +
            computeValue(originalPixel, filterRow + 1, filterCol, pixelRow + 1,
                    pixelCol,
                    pixelRowEnd, pixelColEnd, filterRowEnd, filterColEnd, set, flag, filter) +
            computeValue(originalPixel, filterRow, filterCol + 1, pixelRow,
                    pixelCol + 1,
                    pixelRowEnd, pixelColEnd, filterRowEnd, filterColEnd, set, flag, filter) +
            computeValue(originalPixel, filterRow - 1, filterCol, pixelRow - 1,
                    pixelCol,
  ...
  private double applyBlurOnGreenChannel(PixelInterface[][] originalPixel, int i, int j, int rowEnd,
                                         int colEnd) {
    Set<String> set = new HashSet<>();
    return computeValue(originalPixel, 1,
            1, i, j, rowEnd, colEnd, blur_filter.length, blur_filter[0].length, set, 1,
            blur_filter);
  }

  private double applyBlurOnRedChannel(PixelInterface[][] originalPixel, int i, int j, int rowEnd,
                                       int colEnd) {
    Set<String> set = new HashSet<>();
    return computeValue(originalPixel, 1,
            1, i, j, rowEnd, colEnd, blur_filter.length, blur_filter[0].length, set, 0,
            blur_filter);
  }
